-- Add NSE and BSE 52-week-high price as two more peer metrics alongside
-- Sales/PAT/EBIDTA/Gross Sales Margin/EBIDTA Margin/FII/DII.
--
-- Unlike the other metrics, this isn't derived from a full main_data history
-- of quarters — companies already carries a single current
-- nse_52w_high_price/date and bse_52w_high_price/date per company (the
-- trailing 12-month high as of the data's as-of date), so no materialized
-- "peak across all quarters" view is needed; the companies row itself is
-- the peak record. peak_basis is null (no standalone/consolidated concept
-- for a price series).
...
-- Extend companies_ath (main All-Time High tab) with nse_52w_high_peak/
-- bse_52w_high_peak so the metric dropdown there has real columns to order
-- by, matching the existing *_peak naming convention.
